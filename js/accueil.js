...
window.addEventListener('scroll', () => {
    ...
    // Parallax hero comme avant
    if (hero) hero.style.transform = `translateY(${cur * 0.5}px)`;

    // Comportement de base : hide / show selon le scroll (desktop uniquement)
    if (header && window.innerWidth > 900) {
        const scrollingDown = cur > lastScroll && cur > 100;
        if (!header.classList.contains('header-hover-visible')) {
...
// Affichage au survol en haut de l'écran (desktop)
window.addEventListener('mousemove', (e) => {
    if (!header || window.innerWidth <= 900) return;

    const zoneHeight = 80; // zone de détection depuis le haut

    if (e.clientY <= zoneHeight) {
        header.classList.add('header-hover-visible');
        header.style.transform = 'translateY(0)';
    } else if (!(
        e.clientY <= header.getBoundingClientRect().bottom &&
        e.clientY >= header.getBoundingClientRect().top
    )) {
        // La souris n'est plus dans la zone haute ni sur le header
        header.classList.remove('header-hover-visible');
